...
La elección entre métodos invasivos y no invasivos depende del presupuesto disponible, la profundidad de investigación requerida y el nivel de incertidumbre aceptable. En la práctica geotécnica avanzada, la combinación de ambos (por ejemplo, [[Cross-Hole Test|cross-hole]] + [[MASW Method|MASW]]) es la estrategia óptima: el método invasivo calibra los resultados del no invasivo y reduce la [[Non-uniqueness|no-unicidad]] de la [[Inversión|inversión]] al proporcionar información a priori sobre el perfil.
...
3. **[[Scholte Waves|Ondas de Scholte]]**: ondas de interfaz entre un fluido (agua) y un sólido (sedimentos marinos). Son el equivalente submarino de las [[Rayleigh Waves|ondas de Rayleigh]]. Su [[Phase Velocity|velocidad de fase]] es dispersiva y controlada por la capa de agua y el perfil $V_S$ del fondo marino (véase Sección 8.2).
...
Existe una gran variedad de ensayos de ondas superficiales, identificados con distintos acrónimos ([[SASW Method|SASW]], [[MASW Method|MASW]], [[ReMi Method|ReMi]], [[SPAC Method|SPAC]], [[HVSR|HVSR]]…). Pueden diferir en el tipo de fuente, el número de receptores, las técnicas de procesamiento de señales y las estrategias de [[Inversión|inversión]], pero el principio general es esencialmente el mismo: explotar la [[Dispersion Relation|dispersión geométrica]] de las [[Surface Waves|ondas superficiales]] para inferir el perfil $V_S$.
...
*(Diagrama de flujo de las pruebas de ondas superficiales: adquisición del movimiento del suelo → procesamiento para obtener espectro f–k y [[Dispersion Curve|curva de dispersión]] → [[Inversión|inversión]] para obtener perfil $V_S(z)$ en capas.)*

La mayoría de los ensayos se realizan con **[[Rayleigh Waves|ondas de Rayleigh]]** porque son fáciles de generar con fuentes verticales y de detectar con geófonos verticales en la superficie. Las aplicaciones con [[Love Waves|ondas de Love]] son posibles pero presentan restricciones relacionadas con la estratigrafía y la dificultad de generar ondas SH. Las aplicaciones en el fondo marino emplean [[Scholte Waves|ondas de Scholte]] (véase Sección 1.6.4 y Capítulo 8).
...
**Fuentes de energía**: para generar [[Rayleigh Waves|ondas de Rayleigh]] superficiales se pueden usar diversas fuentes, siempre que aporten energía suficiente en el rango de frecuencias de interés:
...
*(Ejemplo de procesamiento con análisis f–k: (a) datos de campo — sismograma con 48 trazas y 70 m de apertura; (b) espectro f–k con crestas correspondientes al [[Surface Wave Modes|modo fundamental]] de Rayleigh; (c) [[Dispersion Curve|curva de dispersión]] experimental resultante — [[Phase Velocity|velocidad de fase]] vs. frecuencia, de ~200 m/s a 45 Hz hasta ~900 m/s a 10 Hz.)*

El procesamiento puede realizarse directamente en campo con sistemas basados en PC. La inspección visual de la [[Dispersion Curve|curva de dispersión]] antes de la [[Inversión|inversión]] es importante para validar cualitativamente la calidad de los datos.
...
El **juicio de ingeniería** juega un papel importante en todo el procedimiento: los resultados de métodos de [[Inversión|inversión]] automatizados deben examinarse con atención, con especial énfasis en los resultados intermedios de cada paso.
...
Desde la década de 1920 los sismólogos estudiaron ondas superficiales para caracterizar el interior de la Tierra, aunque su uso sistemático se consolidó en los años 1950–1960 con el desarrollo de redes sísmicas de banda ancha (Ben-Menhaem 1995; Aki y Richards 2002). Las [[Rayleigh Waves|ondas de Rayleigh]] son fácilmente identificables en la componente vertical del sismograma; las [[Love Waves|ondas de Love]] en la componente transversal solo pudieron separarse con la llegada del registro digital. El flujo de trabajo en sismología global sigue el mismo esquema que en [[MASW Method|MASW]]: estimación de la [[Dispersion Relation|curva de dispersión]] ([[Group Velocity|velocidad de grupo]] o de fase) seguida de [[Inversión|inversión]] para obtener el perfil de rigidez.

Un desafío central fue la influencia de los [[Surface Wave Modes|modos superiores]]: la presencia de múltiples modos obliga a técnicas sofisticadas de filtrado modal. La [[Inversión|inversión]] multimodal es ventajosa porque los [[Surface Wave Modes|modos superiores]] penetran a mayor profundidad que el fundamental (véase Capítulo 2), y este principio migró a los métodos de caracterización de sitio.
...
> [!EXAMPLE] Evidencia empírica: Paper 003 (Nazarian & Stokoe 1984) — SASW: primer sistema sistemático para Vs(z) in situ con análisis espectral
> **Paper 003 (Nazarian & Stokoe 1984, *Proc. 8th World Conf. Earthquake Engineering*, Vol. III:31–38, >3000 citas)** presenta el **[[SASW Method|método SASW]]** (Spectral Analysis of Surface Waves) como primer procedimiento sistemático para obtener perfiles de [[Shear Wave Velocity|velocidad de onda de corte]] $V_S(z)$ de manera no invasiva. La metodología usa dos receptores a separaciones crecientes: el espectro cruzado de potencia entre señales adyacentes entrega la función de transferencia entre ellas, cuya fase — dividida entre la separación del par — es directamente la curva de [[Phase Velocity|velocidad de fase]] de Rayleigh. La inversión de esta curva por ajuste iterativo de un modelo de capas horizontales produce el perfil $V_S(z)$. Los autores validaron el método en varios sitios de campo en Estados Unidos, mostrando concordancia con sondeos de borehole. Tres limitaciones fundamentales de SASW documentadas en este trabajo motivaron directamente el desarrollo de MASW (Park et al. 1999): (1) **eficiencia de campo baja** — se requieren múltiples configuraciones de receptor con disparos separados, proceso lento y operacionalmente tedioso; (2) **aliasing espacial** — con solo dos receptores no es posible separar los [[Surface Wave Modes|modos superiores]] del modo fundamental; (3) **interpretación subjetiva** — el picking de la curva de dispersión del espectro cruzado es dependiente del operador. MASW resuelve los tres problemas con el arreglo multicanal y el análisis f-k, conservando el mismo equipamiento de bajo costo. Este paper es el punto de partida histórico obligatorio del árbol genealógico de los métodos de ondas superficiales de ingeniería.
>
> — Research Database, entrada 003; Nazarian & Stokoe (1984), *Proc. 8th World Conf. Earthquake Engineering*, San Francisco, Vol. III, pp. 31–38.
...
La cadena metodológica —**[[Adquisición de Datos|adquisición]] → [[Procesamiento de Señales|procesamiento]] → [[Inversión|inversión]]**— introduce fuentes de error y sesgo en cada etapa: el diseño del arreglo puede contaminar la [[Dispersion Curve|curva de dispersión]] con efectos de [[Near-field Effect|campo cercano]]; el procesamiento puede extraer una [[Dispersion Curve|curva aparente]] que mezcla modos sin detectarlos; y la [[Inversión|inversión]] puede quedar atrapada en un mínimo local o reportar un perfil $V_S$ que ajusta bien los datos pero no refleja la geología real. Estos desafíos no invalidan el método, pero imponen requisitos de diseño, criterio profesional e incorporación de información a priori (boreholes, geología local) para producir resultados confiables. Las secciones siguientes cuantifican los cuatro desafíos principales identificados por Foti et al. (2014): superposición modal, efectos de campo cercano, errores de modelo y limitaciones de resolución en profundidad.

### 1.6.1 Superposición Modal y Velocidad Aparente

En un [[Layered Media|medio estratificado]], las [[Rayleigh Waves|ondas de Rayleigh]] se propagan en múltiples modos con [[Dispersion Curve|curvas de dispersión]] independientes (véase [[Surface Wave Modes]]). Con un número limitado de receptores, no es posible separar perfectamente todos los modos: el procesamiento extrae una única [[Dispersion Curve|curva de dispersión]] *aparente* que resulta de la [[Mode Superposition|superposición]] energética de los modos presentes en el campo de ondas.

Cuando el perfil de $V_S$ crece monotónicamente con la profundidad (**medio normalmente dispersivo**), el [[Surface Wave Modes|modo fundamental]] concentra la mayor parte de la energía y la curva aparente coincide razonablemente con él. Sin embargo, cuando hay una [[Inversión|inversión]] de velocidades (capa blanda subsuperficial) o una costra superficial rígida (**medio inversamente dispersivo**), los [[Surface Wave Modes|modos superiores]] pueden ser energéticamente dominantes en ciertas bandas de frecuencia. La Figura 1.9 (p. 22) ilustra cómo la curva aparente de un sitio inversamente dispersivo no corresponde a ningún modo individual, sino a una mezcla que produce una rampa ascendente en velocidad. Invertir esta curva asumiendo que es el [[Surface Wave Modes|modo fundamental]] conduce a un perfil $V_S$ erróneo.
...
Las pruebas de ondas superficiales pueden usarse como herramienta de **monitoreo de la efectividad de técnicas de mejora de suelo**. Repitiendo el ensayo en diferentes momentos del proceso de mejora — por ejemplo, antes y después de vibro-compactación, inyección de lechada o instalación de columnas de grava — se obtiene la evolución temporal del perfil $V_S$. La [[Dispersion Curve|curva de dispersión]] experimental cambia con la rigidez del suelo, y la [[Inversión|inversión]] produce una estimación cuantitativa del efecto de la mejora a cada profundidad.
...
- **Perfil inversamente dispersivo**: la capa asfáltica superficial es más rígida que la base granular y el subrasante. Esto hace que los [[Surface Wave Modes|modos superiores]] sean energéticamente importantes y deba considerarse la [[Mode Superposition|superposición modal]] en la [[Inversión|inversión]].
- **Ventaja informativa**: la geometría del sistema de pavimento es conocida a priori, lo que reduce el número de incógnitas y hace más robusto el proceso de [[Inversión|inversión]].

### 1.7.4 Caracterización Offshore y Near-Shore

> Fuente: Foti et al. (2014), Sección 1.6.4, p. 32.

Las aplicaciones submarinas de los métodos de ondas superficiales se basan en las **[[Scholte Waves|ondas de Scholte]]**, que son el equivalente de las [[Rayleigh Waves|ondas de Rayleigh]] en la interfaz sólido–fluido (fondo marino). Las ondas de Scholte se propagan a lo largo del fondo marino y su [[Phase Velocity|velocidad de fase]] es dispersiva: depende de la frecuencia y está controlada por la profundidad del agua y el perfil $V_S$ de los sedimentos marinos.
...
**Juicio de ingeniería necesario**: los resultados de métodos automáticos de [[Inversión|inversión]] deben revisarse críticamente. El conocimiento de la física de la propagación, la calidad de los datos y la geología del sitio son ingredientes esenciales para una interpretación confiable.

> [!EXAMPLE] Evidencia empírica: Paper 015 (Moffat, Correia & Pastén 2016) — MASW vs. downhole en Chile: 12 geófonos 4.5 Hz producen Vs30 comparables al ensayo invasivo
> **Paper 015 (Moffat, Correia & Pastén 2016, *Obras y Proyectos* No.20, pp. 6–15, SciELO Chile, OA CC BY-NC)** compara los perfiles $V_{S,30}$ obtenidos por [[MASW Method|MASW]] activo, ensayo [[Downhole Test|downhole]] invasivo y bender elements de laboratorio en cinco sitios de Chile central con datos de sondaje disponibles (Maipú, Peñalolén, Casablanca, Melipilla, Llolleo). La configuración [[MASW Method|MASW]] usa **12 [[Geophone|geófonos]] verticales de 4.5 Hz**, procesamiento f-k con Geopsy e [[Inversión|inversión]] estándar de la [[Dispersion Curve|curva de dispersión]] de Rayleigh. Los resultados muestran que el [[Vs30|$V_{S,30}$]] del [[MASW Method|MASW]] concuerda con el del downhole dentro del ±10–15% en la mayoría de los sitios, confirmando la validez del método no invasivo para la clasificación sísmica de suelos según [[Eurocode 8|Eurocode 8]] y NCh2745. Es el primer estudio de validación sistemática [[MASW Method|MASW]] vs. downhole para el contexto geotécnico de Chile, donde las condiciones de suelo (cuencas sedimentarias andinas, alto riesgo sísmico) son análogas a las del contexto paraguayo de esta tesis. La combinación **bajo costo + 12 geófonos + validación en campo** demuestra la viabilidad del enfoque en países con recursos limitados.
> — Research Database, entrada 015; Moffat, Correia & Pastén (2016), *Obras y Proyectos* No.20, pp. 6–15. DOI: 10.4067/S0718-28132016000200001.