...
description: Problema de autovalores diferencial para ondas superficiales en medios continuamente estratificados — la velocidad de fase como autovalor y la distribución de amplitud vertical como autovector — generalización del Thomson-Haskell a perfiles continuos
type: reference
---
...
Este es un problema de Sturm-Liouville cuya solución no trivial existe solo para valores discretos de $k(\omega)$ — la **curva de dispersión** de Love. Para cada frecuencia $\omega$, los autovalores $k_n(\omega)$ con $n = 0, 1, 2, \ldots$ (modo fundamental y superiores) definen la dispersión multimodal.
...
El operador $\mathbf{A}$ depende de $\lambda(x_2)$, $\mu(x_2)$, $\rho(x_2)$, $k$ y $\omega$. Los autovalores del sistema (valores de $c$ que satisfacen las condiciones de contorno) conforman la curva de dispersión de Rayleigh — problema resuelto numéricamente por el [[Thomson-Haskell Matrix|método de Thomson-Haskell]] o la formulación de rigidez.

## Relación con kernels de sensibilidad

Las eigenfunctions $U_n(x_2)$ tienen interpretación física directa: son proporcionales a los **kernels de Fréchet** $\partial c_R/\partial V_S$ — la sensibilidad de la velocidad de fase al módulo de corte a cada profundidad (ver [[Jacobian Matrix]]). Esto hace que el cálculo de los autovectores sea equivalente al cálculo de las derivadas parciales necesarias para la inversión lineal.
