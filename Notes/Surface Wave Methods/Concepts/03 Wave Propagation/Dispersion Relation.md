...
> [!CONCEPT] Definición
> La **relación de dispersión** $\omega(k)$ codifica cómo las propiedades elásticas del medio restringen los pares $(\omega, k)$ posibles para una onda propagante. En medios homogéneos es lineal (no dispersiva); en medios [[Layered Media|estratificados]] es no lineal e implícita: se obtiene como la condición de que el determinante de la matriz de transferencia [[Thomson-Haskell Matrix|Thomson-Haskell]] sea cero. Su evaluación numérica produce la [[Dispersion Curve|curva de dispersión]] teórica que se compara con la experimental en la [[Inversión|inversión]].
>
...
En un semiespacio homogéneo: $\omega = V_P k$ para ondas P y $\omega = V_S k$ para ondas S — relaciones lineales, [[Elastic Half Space|no dispersivas]]. En un [[Layered Media|medio estratificado]], la mezcla de propiedades con la profundidad introduce una no-linealidad: la [[Phase Velocity|velocidad de fase]] $c_p = \omega/k$ depende de $\omega$ (o equivalentemente de $k$).
...
## Relación de dispersión en medios estratificados: [[Thomson-Haskell Matrix|Thomson-Haskell]]
...
donde $\mathbf{T}(\omega, k)$ es la **matriz de transferencia global** de [[Thomson-Haskell Matrix|Thomson-Haskell]], que propaga las condiciones de contorno (continuidad de desplazamiento y tracción) desde el semiespacio inferior hasta la superficie libre. Para cada valor de $\omega$, se buscan los valores de $k$ (o equivalentemente $c_p = \omega/k$) que anulan el determinante.
...
> **Paper 002 (Xia, Miller & Park 1999)** implementa el cálculo de la relación de dispersión teórica via [[Thomson-Haskell Matrix|Thomson-Haskell]] como núcleo del algoritmo de inversión [[MASW Method|MASW]]. El Jacobiano ∂V_R/∂Vs se calcula analíticamente a partir de la relación de dispersión, haciendo la inversión computacionalmente eficiente (< 10 iteraciones en convergencia).
>
