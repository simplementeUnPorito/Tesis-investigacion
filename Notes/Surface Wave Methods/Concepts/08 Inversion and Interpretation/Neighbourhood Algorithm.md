...
- Obtener intervalos de confianza para $[[Vs30|Vs,30]]$ y otros parámetros derivados

> [!EXAMPLE] Evidencia empírica: Wathelet et al. (2004) — NA en inversión de datos de [[MASW Method|MASW]] y arrays pasivos
> Wathelet, Jongmans & Ohrnberger (2004) aplican el Neighbourhood Algorithm a la inversión de [[Dispersion Curve|curvas de dispersión]] de [[MASW Method|MASW]] y arrays pasivos ([[SPAC Method|SPAC]]/[[Beamforming|beamforming]]) en el software **Geopsy/Dinver** — que implementa el NA de Sambridge (1999). En el sitio de Grenoble (depósitos aluviales profundos), el NA evalúa 50,000 modelos $V_S(z)$ en ~10 minutos en hardware de la época (vs. semanas equivalentes con [[Monte Carlo Methods|Monte Carlo]]). El conjunto de modelos aceptables (misfit < umbral) reproduce el perfil de referencia (Cross-Hole) dentro de ±15% en todo el perfil de 50 m, con incertidumbre $\sigma_{[[Vs30|Vs,30]]} \approx 8\%$. La implementación en Geopsy/Dinver se convirtió en el estándar de facto para la inversión de datos de [[Surface Waves|ondas superficiales]] en Europa, adoptada también en los estudios InterPACIFIC (Papers 006, 007).
>
