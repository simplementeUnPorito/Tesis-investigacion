...
description: Problema matemático que no satisface los criterios de Hadamard — la inversión de la curva de dispersión es ill-posed por no-unicidad y falta de estabilidad, requiriendo regularización
type: reference
---
...
> - **Unicidad violada**: múltiples perfiles $V_S(z)$ producen la misma curva de dispersión (dentro de la incertidumbre experimental) → [[Non-uniqueness|no-unicidad]] inherente al problema inverso
...
> La violación de la unicidad es el aspecto dominante: la inversión de MASW produce una **familia de modelos equivalentes**, no un único modelo verdadero. El tratamiento estándar del ill-posedness es la **regularización** ([[Tikhonov Regularization]]), que añade información a priori (suavidad del modelo) para seleccionar una solución única y estable dentro de la familia de modelos equivalentes.
>
> — Foti et al. (2018), §6.4; Tarantola (2005), §1.2; Hadamard (1902).

## Criterios de Hadamard aplicados a MASW
...
1. **Finitud del rango de frecuencias**: la curva de dispersión se mide en $[f_{min}, f_{max}]$ → el perfil $V_S(z)$ solo se puede resolver para $z \in [c_{R,min}/(3f_{max}), c_{R,max}/(3f_{min})]$ → información nula fuera de este rango
...
3. **Insensibilidad a parámetros**: $c_R$ es muy insensible a $V_P$ (en suelos saturados) y a $\rho$ → estos parámetros no pueden determinarse por MASW → se deben fijar a priori

## Regularización como tratamiento estándar
...
|--------|-----------------|
| Foti et al. (2018), *Surface Wave Methods* | §6.4, pp. 285–295 — ill-posedness en inversión MASW |
