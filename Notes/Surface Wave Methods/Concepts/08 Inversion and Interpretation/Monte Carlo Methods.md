...
El desajuste (*misfit*) entre la [[Dispersion Curve|curva de dispersión]] teórica $\mathbf{d}^{cal}(\mathbf{m})$ y la experimental $\mathbf{d}^{obs}$ se define como (Foti Ec. 6.34):
...
2. Calcular la [[Dispersion Curve|curva de dispersión]] teórica de cada modelo ([[Forward Problem|problema directo]]).
...
- **Resultado práctico**: aunque los perfiles difieren, los espectros de respuesta 1D calculados con cada perfil son prácticamente idénticos (Fig. 7.21, p. 373). La [[Non-uniqueness|no-unicidad]] no se traduce en incertidumbre práctica para la respuesta sísmica.
...
| Cuantificación de [[Non-uniqueness|no-unicidad]] | No | Sí (conjunto de soluciones) |
...
- Costo computacional elevado: $10^4$–$10^6$ evaluaciones del [[Forward Problem|problema directo]].
...
| Olafsdottir, Erlingsson & Bessason (2020), *Geosciences* 10(8):322 | Paper 013 — MASWaves: herramienta open-source MATLAB para inversión Monte Carlo [[MASW Method|MASW]]; CC BY 4.0 |
