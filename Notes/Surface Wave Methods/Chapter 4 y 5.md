...
En un medio no dispersivo, las ondas se propagan con una sola velocidad y la forma de onda no cambia al propagarse. Cuando existe [[Wave Dispersion|dispersión]] —como en las [[Surface Waves|ondas superficiales]] en medios heterogéneos— la velocidad depende de la frecuencia y la forma de onda cambia con la distancia (Rayleigh 1877). En este caso coexisten dos conceptos de velocidad (Foti Ec. 4.1):
...
Dado que $\theta_{12}$ solo se mide entre $-\pi$ y $\pi$, es necesario un proceso de **[[Phase Unwrapping|phase unwrapping]]** (desenvoltura de fase) para recuperar la fase absoluta.

La **[[Coherence Function|función de coherencia]]** $\gamma^2_{12}(\omega)$ actúa como indicador de calidad: valores próximos a 1 indican que la señal es coherente entre receptores y el resultado es confiable.
...
Esto evita tanto el [[Near-field Effect|campo cercano]] (longitudes de onda muy grandes) como la pérdida de resolución espacial (longitudes de onda muy pequeñas). Para cubrir un amplio rango de frecuencias es necesario repetir el ensayo con múltiples separaciones $D$.
...
[[MOPA]] (*Multi-Offset Phase Analysis*) es la extensión multicanal de [[SASW Method|SASW]]. En lugar de dos receptores, se usa un array completo. Bajo la hipótesis de un **único modo de propagación**, la fase en el receptor $i$ a posición $x_i$ es:

$$\varphi_i(\omega) = -k(\omega) \cdot x_i + \varphi_0(\omega)$$

donde $k(\omega)$ es el [[Wavenumber|número de onda]] y $\varphi_0(\omega)$ es la fase en el origen. Esto define un sistema lineal matricial:
...
- Ofrece un **test estadístico de linealidad** ([[Chi-Squared Test|test chi-cuadrado]], Ec. 4.26–4.27) para evaluar automáticamente si la hipótesis de modo único y campo lejano se cumple.
...
donde $\chi$ es el lag espacial y $E[\cdot]$ es el valor esperado. Esta expresión es la **transformada inversa de Fourier del espectro de [[Wavenumber|número de onda]]**: la autocorrelación espacial codifica directamente el contenido espectral del campo de ondas.
...
donde $J_0$ es la [[Bessel Function|función de Bessel de orden cero de primera especie]] y $r$ es la separación entre receptores. Esta es la expresión original de Aki (1957). La estimación experimental usa un receptor de referencia en el origen del array con la expresión (Foti Ec. 4.41):
...
**Limitación:** requiere receptores equiespaciados. Para arrays no equiespaciados se emplea *[[Beamforming|frequency-domain beamforming]]*:
...
**Comparación con f–k:** la imagen en frecuencia–lentitud tiende a ser más nítida e intuitiva. [[MASW Method|MASW]] (McMechan y Yedlin 1981) usa formalmente la transformada τ–p como base.

#### 4.6.3 ReMi (Refraction Microtremor)

[[ReMi Method|ReMi]] (Louie 2001) adapta la metodología f–k al caso de **mediciones pasivas con arrays lineales**. Al usar ruido ambiental, la dirección de propagación de las fuentes es desconocida. Para una onda plana con ángulo $\vartheta$ respecto al array:
...
En un medio [[Viscoelastic Media|viscoelástico]], el [[Wavenumber|número de onda]] es complejo:
...
El método del **ancho de banda a mitad de potencia** (Badsar et al. 2010) extrae las curvas de atenuación directamente a partir de los **[[2D Fourier Transform|espectros f-k]]** utilizados en el análisis de dispersión, sin requerir un análisis separado de amplitudes en el dominio espacio-offset.
...
La **[[Arias Intensity|intensidad de Arias]]** de una señal sísmica es proporcional a la integral del cuadrado de la aceleración en el tiempo:
...
El análisis de dispersión produce la curva de [[Phase Velocity|velocidad de fase]] aparente $V_R(f)$. La elección del método determina qué información se puede extraer:
